...
# Copy over relevant files to `\\publish\\glsl-lsp-vscode\\$EXT_VERSION`.
...
New-Item -Path (Join-Path $script:ROOT "publish") -Name "glsl-lsp-vscode" -ItemType Directory -Force -Verbose
$script:OUT_DIR = New-Item -Path (Join-Path $script:ROOT "publish" "glsl-lsp-vscode") -Name "$ext_version" `
